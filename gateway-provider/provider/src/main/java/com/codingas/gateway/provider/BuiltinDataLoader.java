...
public class BuiltinDataLoader implements CommandLineRunner {
    ...
    private String upsertProvider(Provider provider) {
        return providerRepository.findByCode(provider.getCode())
                .map(existing -> {
    ...
    private String upsertPlan(PlanCatalog catalog) {
        return planCatalogRepository.findByPlanCode(catalog.getPlanCode())
                .map(existing -> {
                    copyPlanFields(catalog, existing);
                    planCatalogRepository.save(existing);
                    return "UPDATED";
                })
                .orElseGet(() -> {
                    planCatalogRepository.save(catalog);
                    return "ADDED";
                });
    }
    ...
    private String upsertPlanModel(PlanModelCatalog catalog) {
        return planModelCatalogRepository.findByPlanCodeAndModelName(
                        catalog.getPlanCode(), catalog.getModelName())
                .map(existing -> {
                    planModelCatalogRepository.save(existing);
                    return "UPDATED";
                })
                .orElseGet(() -> {
                    planModelCatalogRepository.save(catalog);
                    return "ADDED";
                });
    }
    ...
    private String upsertModel(Model model) {
        return modelRepository.findByModelName(model.getModelName())
                .map(existing -> {
