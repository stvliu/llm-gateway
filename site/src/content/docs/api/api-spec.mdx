...
> 认证使用 `Authorization: Bearer <网关 API Key>`（以 `sk-` 开头）。完整可运行示例见 [API 调用示例](/api/examples/)。
...
> 认证使用 `x-api-key` 头（非 Bearer），且必须携带 `anthropic-version: 2023-06-01`。完整示例见 [API 调用示例](/api/examples/)。
