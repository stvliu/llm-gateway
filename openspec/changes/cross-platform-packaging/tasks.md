...
- [x] 1.2 新增 Linux 启动脚本 `deployments/package/bin/llmgateway.sh`（source conf + exec java $JAVA_OPTS -jar）
- [x] 1.3 改 `deployments/package/linux/llmgateway.service`（ExecStart 指向 llmgateway.sh，去掉 EnvironmentFile）
...
- [x] 2.3 配 deb/rpm packager（requires/conffile/scripts 共用 postinst/prerm/postrm）+ archive 出 Windows zip（fileSets: Windows JRE + WinSW + ps1 + conf + jar）

## 3. maintainer 脚本迁移

- [x] 3.1 改 postinst：生成 conf（密钥 `head -c 32 /dev/urandom | base64` + sed `|` 分隔符）+ `chmod 640` conf + `chmod -R 0755 runtime/bin` 兜底 JRE 权限
- [x] 3.2 改 prerm/postrm：适配 conf 与 systemd unit 新布局（保留数据目录 `/var/lib/llmgateway`）
...
- [x] 4.2 新增 `deployments/package/windows/llmgateway.xml`（WinSW service 配置）+ 准备 WinSW.exe（v2.x，作为 archive fileSet）
...
- [x] 5.2 删除 `deployments/package/build.ps1`（Windows 改 zip，不再用 jpackage+iscc）
...
- [x] 7.1 删除 `deployments/package/linux/llmgateway.templates` + `llmgateway.config`（debconf）
...
- [x] 8.4 zip 在 Windows 解压 + install.ps1，Get-Service llmgateway Running，`/actuator/health` 200
- [x] 8.5 升级验证：deb/rpm 升级后 conf 保留（端口/密钥不变），`/var/lib/llmgateway` 数据不丢
- [x] 8.6 卸载验证：apt/dnf remove 或 zip uninstall.ps1 后 `/var/lib/llmgateway` 数据目录保留
- [x] 8.7 jlink 平台验证（design §4.3）：确认交叉生成 Linux JRE 方案可行（ELF 64-bit x86-64，62M）

## 9. 重命名

- [x] 9.1 统一重命名 llm-gateway -> llmgateway（构建链路 + 文档，用户 2026-07-16 决策；commit a3ef7245 + 本任务文档同步）
