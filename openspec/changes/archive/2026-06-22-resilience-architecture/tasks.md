...
- [x] 1.1 新增 `Application` 根实体实体 + `applications` 表（code/name/description/state + 审计字段 + 预留配额/看板字段留空）
...
- [x] 5.3 文档更新（容灾方案设计/管理范式与实现对齐）
  - 提交 a3b51fc；容灾方案设计.md + 容灾管理范式.md 与实现对齐
  - 关键对齐：转移事件流 D12 章节（@EventListener 同步持久化，非 @TransactionalEventListener，含原因）、实现版错误分流表/画像字段/解析链/RouterChain 顺序、控制台三屏实际实现、应急操作端点
  - 设计探讨差异显式标注；无 TBD/占位
