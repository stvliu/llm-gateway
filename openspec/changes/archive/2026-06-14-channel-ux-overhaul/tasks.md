...
- [x] 3.3 验证创建过程任意步骤失败均整体回滚（含级联 Provider / 级联 Model 失败用例）
- [x] 3.4 provision API 集成测试：覆盖正常路径、回滚路径、providerCode 已存在时忽略 inlineProvider
...
- [x] 5.2 改造 EndpointSection.tsx 错误反馈，至少补齐 62-63 行与 89-90 行两处
- [x] 5.3 改造 CredentialSection / ModelMappingSection / QuotaSettingsSection 的 catch 块
- [x] 5.4 单元测试覆盖错误路径
...
- [x] 7.7 useSavePulse.test.tsx 单元测试 + EndpointSection / CredentialSection 组件测试覆盖成功 / 失败两路径
...
- [x] 9.9 HealthDot.test.tsx 组件测试 + Playwright e2e/health-check-matrix.spec.ts (S5)
...
- [x] 10.6 主页面顶部移除独立的"+ 新增供应商"按钮，相关引用清理
- [x] 10.7 QuickOnboardMode.test.tsx 状态机测试（提交 payload 含 inlineProvider）+ Playwright e2e/onboard-inline-provider.spec.ts (S1)

## 11. 国际化与文案统一

- [x] 11.1 整理本期所有新增 / 修改的 i18n key（中英文）
- [x] 11.2 文案审校：危险操作 description / 状态 Tooltip / 保存反馈 / 错误反馈
- [x] 11.3 codegraph 验证文案 key 全部被引用，无孤立 key

## 12. 联调与回归

- [x] 12.1 前后端联调：覆盖 9 条端到端验收场景（S1-S7 + 内联创建 + 错误反馈）
- [x] 12.2 视觉走查（人工确认：状态色/脉冲/矩阵 Table 在 1280×800 呈现；ProviderHeader 聚合保留决策）
- [x] 12.3 Playwright e2e/delete-key-confirm.spec.ts (S6)
- [x] 12.4 回归测试：旧路径（批量导入、模板创建、批量导出）不受影响
- [x] 12.5 准备验证报告草稿，列出每条 Requirement 的覆盖测试
