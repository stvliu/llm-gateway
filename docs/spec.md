...
### 8.11 合规冲突解决：GDPR 删除权 vs WORM 审计
...
| 威胁类别 | 识别威胁 | 缓解措施 | 对应需求 |
|---------|---------|---------|---------|
| **Spoofing** | 伪造 Token 访问 | JWT/Bcrypt Token 验证、IP 白名单 | §8.1 [2], [6] |
| | 伪造上游 Provider | BaseURL 白名单、TLS 证书校验 | §8.1 [1] |
| **Tampering** | 篡改渠道配置 | 乐观锁 + 审计链 + RBAC | data-model.md §FK Cascade |
| | 篡改审计日志 | WORM 存储 + 链式哈希 | §8.6 |
| **Repudiation** | 否认操作行为 | 完整审计链（操作人/IP/时间/详情） | §8.6 |
| **Information Disclosure** | PII 泄露 | PII 全链路脱敏 + 加密 | §8.5, §8.2 |
| | 密钥泄露 | AES-256 加密 + Vault 集成 | §8.4 |
| **Denial of Service** | 超频请求耗尽资源 | RPM/TPM 限流 + 熔断 | §8.1 [9], §4.2.3 CH-014 |
| | 恶意大请求体 | 请求体大小限制（10MB） | §4.2.1 GW-009 |
| **Elevation of Privilege** | 越权访问其他用户数据 | RBAC + GatewayApiKey 关联 | §6.3, §8.9 |
| | 越权访问管理功能 | RBAC + ABAC | §8.9 |
