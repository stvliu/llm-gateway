...
| **API Portal** | API 门户 | P2 | 服务发布、浏览、订阅（企业版） |
| | 订阅审批 | P2 | 手动审批 + 自动审批（企业版） |
| | Prompt 服务 | P2 | 一键将 Prompt 封装为 REST API（企业版） |
...
| GW-011 | 自定义响应头 | 支持配置自定义 HTTP 响应头（如 `X-Correlation-ID`） | 支持静态值和变量注入 |
...
**需求描述**: 对相同请求识别并返回缓存答案，降低延迟和上游调用成本。基于请求内容精确匹配（非语义匹配）。

**功能需求**:

| ID | 功能 | 详细描述 | 验收标准 | 版本 |
|----|------|---------|---------|------|
| CM-001 | 请求缓存 | 将请求及响应缓存，相同请求直接返回缓存 | 匹配基于请求体精确哈希 | Phase 2 |
| CM-002 | 缓存 TTL | 缓存可设置过期时间，避免返回过时答案 | 默认 TTL 1 小时，可配置 | Phase 2 |
| CM-003 | 缓存命中统计 | 统计缓存命中率、节省的 Token 数量 | 实时仪表盘可展示 | Phase 2 |
| CM-004 | 缓存作用域 | 缓存可按用户隔离 | 跨用户缓存不共享 | Phase 2 |
| CM-005 | 缓存失效 | 支持手动清除缓存条目 | API 接口和 UI 可操作 | Phase 2 |
| CM-006 | 流式缓存绕过 | 流式请求可配置是否跳过缓存 | 默认流式不缓存 | Phase 2 |
