...
> **文档版本**: v6.9
> **项目版本**: v1.0.0
> **生成日期**: 2026-05-07（2026-08-31 架构对齐修订）
> **状态**: 草案
...
| | 语音合成端点 | P1 | 标准版 | `/v1/audio/speech` (TTS) |
| | 语音识别端点 | P1 | 标准版 | `/v1/audio/transcriptions` (STT) |
| | 内容审核端点 | P1 | 标准版 | `/v1/moderations` |
| **Provider 管理** | Provider CRUD | P0 | 标准版 | 创建/查询/编辑/删除 |
| | 多 Key 管理 | P0 | 标准版 | 单渠道多 API Key 自动轮换、调度、故障切换 |
| | 负载均衡 | P0 | 标准版 | 优先级 + 权重 |
| | 渠道分组 | P1 | 标准版 | 按用途/价格分组 |
| | 渠道级故障转移 | P1 | 标准版 | 渠道失败时自动重试其他渠道 |
| | 熔断超时 | P1 | 标准版 | 防雪崩 |
| | 代理配置 | P1 | 企业版 | HTTP/S、Socket5 |
| **智能路由** | 模型级智能降级 | P1 | 标准版 | 额度不足/模型不可用时自动切换备选模型 |
| | 场景路由 | P1 | 标准版 | 按场景(CODE/CREATIVE/SUMMARY等)选择模型 |
| | 模型别名映射 | P0 | 标准版 | 将请求中的模型别名映射到实际模型，支持模型→Provider 路由 |
| | 可视化策略编排 | P1 | 企业版 | 零代码策略配置 |
| | 自定义脚本扩展 | P1 | 企业版 | 高级用户自定义逻辑 |
| **语义缓存** | 语义缓存 | P1 | 企业版 | 相似请求返回缓存，降低成本 |
| | 缓存 TTL | P1 | 企业版 | 缓存过期时间配置 |
| | 缓存命中率统计 | P1 | 企业版 | 实时统计缓存效果 |
| **用户管理** | 用户 CRUD | P0 | 标准版 | 创建/查询/编辑/删除用户 |
| | 用户登录/登出 | P0 | 标准版 | 用户名密码登录、会话管理 |
| | 基础 OAuth 登录 | P1 | 标准版 | GitHub、QQ、企业微信 |
| | 企业 OAuth 登录 | P1 | 企业版 | 飞书、钉钉、GitHub Enterprise |
| **密钥管理** | API Key CRUD | P0 | 标准版 | 创建/查询/编辑/删除 |
| | 额度限制 | P1 | 标准版 | API Key 用量上限 |
| | 模型白名单 | P1 | 标准版 | 限制可访问模型 |
| | IP 限制 | P1 | 标准版 | API Key 级 IP 白名单 |
| | 过期时间 | P1 | 标准版 | API Key 有效期 |
| **Token 计量** | Token 计量 | P1 | 标准版 | 输入/输出 Token 分别统计 |
| **Token额度** | Token限额（用户级/API Key级） | P1 | 标准版 | 用户/API Key Token 用量控制 |
| | 用户×渠道限额 | P1 | 企业版 | 精细化配额控制 |
| | 请求次数配额 | P1 | 企业版 | 时间周期内请求次数限制 |
| **安全与风控** | 认证中间件 | P0 | 标准版 | Token 验证 |
| | IP 白/黑名单 | P1 | 标准版 | 访问控制 |
| | UA 过滤 | P1 | 标准版 | User-Agent 过滤 |
| | PII 脱敏 | P1 | 标准版 | 自动检测并脱敏 |
| | 数据掩码策略 | P1 | 标准版 | 敏感数据过滤脱敏 |
| | 审计日志 | P1 | 标准版 | 全操作记录 |
| | 密钥加密存储 | P1 | 标准版 | AES-256 加密 |
| | 国密算法 | P1 | 企业版 | SM2/SM3/SM4 |
| | 完整审计链 (WORM) | P1 | 企业版 | 不可篡改审计日志 |
| **可观测性** | Trace ID | P1 | 标准版 | 全链路追踪 |
| | 结构化日志 | P1 | 标准版 | JSON 格式 |
| | 实时指标 | P1 | 标准版 | 延迟/QPS/Token/费用 |
| | Prometheus 导出 | P1 | 标准版 | 标准指标格式 |
| | Grafana 仪表盘 | P1 | 企业版 | 预配置可视化 |
| | Jaeger 追踪 | P1 | 企业版 | 分布式追踪可视化 |
| **日志管理** | 调用日志 | P1 | 标准版 | API 调用元数据 |
| | 请求体日志 | P1 | 标准版 | 完整请求/响应 |
| | 操作日志 | P1 | 标准版 | 管理操作审计 |
| | 日志归档 | P1 | 企业版 | 定期归档清理 |
| **模型管理** | 模型目录 | P1 | 标准版 | 内置模型列表（50+） |
| | 远程同步 | P1 | 标准版 | 同步官方模型数据 |
| | 供应商管理 | P1 | 标准版 | 供应商 CRUD |
| **MCP 支持** | 系统级 MCP | P1 | 标准版 | 统一入口访问所有已发布服务 |
| | 服务级 MCP | P1 | 标准版 | 单个服务启用 MCP |
| **插件系统** | 插件框架 | P1 | 企业版 | SPI 机制 |
| | MCP 协议支持 | P1 | 企业版 | Model Context Protocol |
| **管理控制台** | 登录页面 | P0 | 标准版 | 用户登录/登出界面 |
| | 渠道管理页面 | P0 | 标准版 | Provider CRUD 操作界面 |
| | API Key 管理页面 | P0 | 标准版 | API Key CRUD 操作界面 |
| | 用户管理页面 | P0 | 标准版 | 用户 CRUD 操作界面 |
| | 仪表盘 | P1 | 标准版 | 总体统计与趋势 |
| | 日志查询页面 | P1 | 标准版 | 多维度日志检索 |
| | 系统设置页面 | P1 | 标准版 | 全局配置 |
| | 策略编排页面 | P1 | 企业版 | 可视化策略配置 |
| | 额度配置页面 | P1 | 企业版 | Token限额管理 |
| **部署** | Docker Compose | P1 | 标准版 | 一键部署 |
...
| KP-008 | 默认 Key 保护 | 禁止删除渠道中最后一个 API Key，确保渠道始终有可用凭证 | 删除最后一个 Key 时返回错误，错误消息明确说明"无法删除最后一个 Key" | 标准版 |
| KP-009 | 扩展 Key 状态 | 支持 RATE_LIMITED（限流中）/OVERQUOTA（超配额）/ERROR（异常）状态 | 状态枚举包含新状态，状态变更记录日志，不同状态有明确的禁用原因 | 标准版 |
| KP-010 | 可视化优先级调整 | 前端支持拖拽调整 Key 优先级顺序，后端自动计算 priority 值 | 前端支持拖拽排序，后端提供批量更新优先级 API，优先级变更立即生效 | 标准版 |
| KP-011 | Key 级自动故障转移 | Key 失效时自动切换到下一个可用 Key，保证请求不中断 | Key 失效时切换延迟 ≤100ms，记录切换事件到日志，按优先级顺序选择备用 Key | 标准版 |
| KP-012 | 健康检测与自动恢复 | 定期检查被禁用 Key 的可用性，自动恢复正常的 Key | 默认恢复检查间隔 300 秒，可配置 60-86400 秒，恢复后状态变更为 ACTIVE，记录恢复事件 | 标准版 |
| KP-013 | 多策略 Key 选择 | 支持多种 Key 选择策略：PRIORITY_FIRST（优先级优先）/ROUND_ROBIN（轮询）/WEIGHTED（加权） | 支持三种选择策略，渠道级配置选择策略，策略变更立即生效 | 标准版 |

> **说明**: KP-001~KP-013 的功能实现详见 2.2.7 渠道管理模块（CH-007~CH-013），此处仅作为密钥池视角的需求汇总，避免实现细节重复。
...
**技术实现**:
- Embedding 模型：独立 Embedding 服务（规划中）
...
| **Phase 7** | **预警与计量**：AlertRule 管理、AlertNotification 管理、UsageLog 记录、预警引擎、用量报表 |
| **Phase 8** | **其他功能**：内容安全审核（企业版）、License 管理（企业版） |
...
> **说明**: MVP（最小可用产品）功能由多个 Phase 迭代交付。下表列出 v1.0 发布的最小功能集，确保覆盖所有 P0 核心功能。P0 定义为基础功能，使 LLM-Gateway 可以正常运行和使用。
...
| **模型路由** | 模型别名映射、模型→Provider 路由 | P0 | 标准版 | Phase 3 | 请求 `gpt-4o` 正确路由到 OpenAI Provider |
| **用户管理** | 用户 CRUD、登录/登出 | P0 | 标准版 | Phase 2 | 支持用户名密码登录、会话管理 |
| **API Key 管理** | API Key CRUD | P0 | 标准版 | Phase 2 | API Key 格式: `sk-xxxxxxxx` |
| **Token 认证** | Bearer Token 验证 | P0 | 标准版 | Phase 3 | 无效 Token 返回 401 |
| **管理控制台** | 登录页面、渠道管理页面、API Key 管理页面、用户管理页面 | P0 | 标准版 | Phase 4 | 基础 CRUD 可用 |

**MVP 功能统计**：
- 总计 **9 个核心功能模块**，覆盖 **所有 P0 基础功能**
- 涉及 Phase 2~Phase 4（Phase 1 基础框架、Phase 5-8 增强功能延后到 v1.1+）
- 全部为**标准版**功能，企业版功能在 v1.1+ 迭代

**P1 功能规划**（v1.1+ 迭代）：
- **安全增强**: IP 白/黑名单、PII 脱敏、密钥加密存储、审计日志
- **可观测性**: Trace ID、结构化日志、Prometheus 导出、实时指标
- **Token 管理**: Token 计量、Token 限额（用户级/API Key级）
- **智能路由**: 场景路由、模型级智能降级
- **管理增强**: 仪表盘、日志查询页面、系统设置页面
...
| 版本 | 日期 | 变更内容 | 变更人 |
|------|------|---------|--------|
| v6.9 | 2026-08-31 | **架构对齐修订**：2026-05 后的功能演进（模块化重构、通道生命周期、韧性架构、密钥应用绑定、模型/通道复制、系统设置域、目录同步等）经 OpenSpec 变更管理与 ADR-0001 记录落地；本规格维持需求基线定位，架构事实以 `docs/constitution.md`、`docs/技术架构.md`、`docs/数据架构.md` 与 `openspec/specs/` 为准 | - |
...
| v5.0 | 2026-04-30 | 删除架构重复内容（业务模型、数据模型）；删除 Clarifications；精简技术栈信息 | - |
