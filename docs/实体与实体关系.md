...
|------|-------------|---------|
| **目标用户** | 企业中台/政企客户 | 企业 API 开放平台/多租户 SaaS |
| **核心差异** | 智能路由/负载均衡/故障转移 | API 生命周期管理/订阅审批 |
| **用户体系** | User | User + UserRole + Role (RBAC) |
| **计费模式** | Token 限额（用户级别） | 流量配额管理 |
| **路由策略** | RouteGroup（全局负载均衡/故障转移） | Channel |
...
**业务说明**: 所有用户共享全局的 Provider、RouteGroup 等资源。
...
| 财务管理员 | FINANCE_ADMIN | Token 限额配置、用量查看 |
...
| v5.0 | 2026-04-23 | 重构：移除 Team/Channel/ChannelKey，Member 改名为 UserRole，新增 RouteGroup/RouteGroupProvider/GatewayApiKey |
