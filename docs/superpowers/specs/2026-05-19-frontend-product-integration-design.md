...
| 约束 | 说明 | 前端验证 |
|------|------|---------|
| 一个供应商至少一个产品 | 供应商创建时必须创建默认产品 | 创建供应商向导包含产品创建步骤 |
| 一个产品至少一个 BaseUrl | 产品的 `endpoints` 不能为空 | 表单验证：至少配置一个协议端点 |
| 一个产品至少一个 API Key | 产品必须有可用的 Key 才能调用 | 创建产品时必须至少添加一个 Key |

### 约束落地

1. **供应商创建**：现有 `ProviderCreateModal` 已包含嵌套 API Key 和模型创建，需扩展支持产品创建
2. **产品表单验证**：
   - `endpoints` 至少一个条目（默认协议：`openai`）
   - `apiKeys` 至少一个条目
3. **删除保护**：
   - 供应商下只有一个产品时，禁止删除该产品
   - 产品下只有一个 API Key 时，禁止删除该 Key
...
7. **供应商创建扩展** — 修改 ProviderCreateModal 支持产品创建（可选，视后端接口而定）
...
- 国际化文本正确显示
- **业务约束验证**：
  - 创建产品时必须填写至少一个 BaseUrl
  - 创建产品时必须至少添加一个 API Key
  - 供应商只有一个产品时禁止删除
  - 产品只有一个 API Key 时禁止删除
